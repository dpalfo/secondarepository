				...
				for (Direttore i : magazzino.getDirettori()) {
					if (codice.equals(i.getCodic())) {
						do {
							...
							System.out.println(
									"1) Assumi autista - 2) Assumi operaio - 3) Aggiungi merce - 4) Aggiungi mezzo\n"
						...
						if (!magazzino.getAutisti().isEmpty()) {
							for (Autista a : magazzino.getAutisti()) {
								if (b == a.getId()) {
									System.out.println("Benvenuto " + a.getNome()
											+ " vuoi vedere le caratteristiche dei mezzi da te guidati ?");
